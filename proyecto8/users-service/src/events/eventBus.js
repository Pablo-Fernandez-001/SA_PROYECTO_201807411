  ...
  async connect() {
    ...
    for (let attempt = 1; attempt <= 10; attempt += 1) {
      try {
        ...
        conn.on("close", () => {
          this.logger.warn("RabbitMQ connection closed (users-service)");
          this.channel = null;
        });
        this.channel = await conn.createChannel();
        await this.channel.assertExchange(this.exchange, "topic", { durable: true });
        this.logger.info({ attempt }, "Connected to RabbitMQ (users-service)");
        return;
      } catch (error) {
        this.logger.error(
          { attempt, error: error.message },
          "RabbitMQ connection failed (users-service)"
        );
        await new Promise((resolve) => setTimeout(resolve, 1000 * attempt));
      }
    }

    this.logger.error("RabbitMQ unavailable after retries (users-service)");
  }
